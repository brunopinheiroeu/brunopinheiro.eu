  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 mx-auto grid max-w-6xl items-center gap-12 px-6 md:grid-cols-2">
        ...
        >
          {imgs.map((img, i) => {
            // Each card has different influence (depth) for a layered parallax feel
            const depth = [1.0, 0.85, 0.75, 0.9, 0.8][i];
            const tRotX = useTransform(rotX, (v) => v * depth);
            const tRotY = useTransform(rotY, (v) => v * depth);
            const tX = useTransform(shiftX, (v) => v * depth);
            const tY = useTransform(shiftY, (v) => v * depth);
            ...
            return (
              <motion.div
                ...
                // Base 3D rotation + parallax transforms (no jumps)
                style={{
                  rotateX: tRotX, // global tilt (scaled by depth)
                  rotateY: tRotY,
                  x: tX,
                  y: tY,
                }}
                whileHover={{
                  // straightens a bit + lifts on hover (touch-like “arrange”)
                  rotateX: 0,
                  rotateY: 0,
                  rotateZ: 0,
                  y: -6,
        ...
        <div className="relative grid grid-cols-2 gap-4 md:hidden">
          {imgs.slice(0, 4).map((img) => (
            <div
              key={img.src}
              className="h-40 w-full rounded-2xl bg-white/10 shadow-[0_10px_24px_rgba(0,0,0,0.22)] ring-1 ring-white/20"
            >
              <img
                src={img.src}
                alt={img.alt}
